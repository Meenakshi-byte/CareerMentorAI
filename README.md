...
Career Mentor AI is a compact full-stack application that helps students discover the best-fit technology careers by combining a small Random Forest model with a clean React + Tailwind frontend and a lightweight Flask API.
...
- Backend: Python, Flask, Flask-CORS, Flask-JWT-Extended, Flask-SQLAlchemy, SQLAlchemy
...
1) Create a Python virtual environment locally and install backend deps
...
pip install flask flask-cors flask-jwt-extended flask_sqlalchemy sqlalchemy pandas numpy scikit-learn
```

This creates a local `.venv` folder on your machine. It is not pushed to Git, and each clone should create its own virtual environment.
...
Note: run the backend from the repository root with `python -m backend.run` to ensure the `backend` package imports resolve correctly.
...
If you clone from GitHub, you can run the backend without a `.env` file because these values fall back to safe defaults. The README instructions are complete for running locally.

If you want to override defaults, export environment variables in your shell before starting the app.
...
- When the backend starts, it loads ML artifacts from `backend/ml/artifacts` and prints the available role list.
- You may also see a scikit-learn version warning because the saved model artifacts were created with an older scikit-learn version than the current environment.
